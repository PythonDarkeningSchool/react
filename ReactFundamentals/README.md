...
```react
class Form extends React.Component {
  userNameInput = React.createRef(); // (2) an object must created when declare an ID
  handleSubmit = (event) => { // event handler
    event.preventDefault(); // (4) prevent form submission
    console.log(this.userNameInput.current.value); // get value from input from the ID
  }
  
	render() {
  	return (
    	<form onSubmit={this.handleSubmit}>  // (3) create onSubmit event
    	  <input 
          type="text" 
          placeholder="GitHub username"
          ref={this.userNameInput} // (1) fancy way to declare an ID in react
          required // (5) make this field as required
          />
        <button>Add card</button>
    	</form>
    );
...
This has more advantages over read it from the DOM but does require more code (check the comments in the below code)

```react
class Form extends React.Component {
  state = {userName: ''}; // (1) introduce a new json object
  handleSubmit = (event) => { // event handler
    event.preventDefault(); // (4) prevent form submission
    console.log(this.state.userName); // (6) accessing to the user's value every time
  };
	...
	render() {
  	return (
    	<form onSubmit={this.handleSubmit}> 
          ...
          type="text" 
          placeholder="GitHub username"
          value={this.state.userName} // (2) read the userName from the json object
          onChange={event => this.setState({userName: event.target.value})} // (3) allows to write in input field
          required // (5) make this field as required
          />
        <button>Add card</button>
    	</form>
